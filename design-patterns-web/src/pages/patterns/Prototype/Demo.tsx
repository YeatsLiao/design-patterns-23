    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
        ...
        <div className="space-y-2">
          {shapes.map(shape => (
            <div key={shape.id} className="flex items-center justify-between bg-gray-900 p-3 rounded-lg border border-gray-800">
               <div className="flex items-center gap-3">
                 <div className={clsx("w-6 h-6", shape.type === 'circle' ? 'rounded-full' : 'rounded-sm', shape.color)}></div>
                 <span className="text-sm text-gray-300 font-mono">{shape.id} ({shape.type})</span>
               </div>
               <button 
        ...
        <AnimatePresence>
          {shapes.map((shape) => (
            <motion.div
              ...
              className={clsx(
                "absolute w-16 h-16 shadow-lg flex items-center justify-center text-white/50 cursor-grab active:cursor-grabbing hover:ring-2 ring-white/30",

  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
        <h3 className="text-xl font-semibold text-white mb-6">Forest Renderer</h3>
        
        <div className="space-y-4 mb-8">
           <button onClick={() => plantForest(10)} className="w-full py-2 bg-gray-700 hover:bg-gray-600 rounded text-white">Plant 10 Trees</button>
           <button onClick={() => plantForest(100)} className="w-full py-2 bg-gray-700 hover:bg-gray-600 rounded text-white">Plant 100 Trees</button>
           <button onClick={() => plantForest(1000)} className="w-full py-2 bg-blue-600 hover:bg-blue-500 rounded text-white font-bold">Plant 1,000 Trees</button>
           <button onClick={clear} className="w-full py-2 bg-red-900/50 hover:bg-red-900/70 text-red-200 rounded">Clear Forest</button>
        </div>
        ...
        <div className="bg-gray-950 p-4 rounded-lg font-mono text-xs">
           ...
           <div className="flex justify-between mb-2">
             <span className="text-gray-400">Est. Memory (Naive):</span>
             <span className="text-red-400">{memoryUsage.withoutFlyweight.toLocaleString()} units</span>
           </div>
           <div className="flex justify-between border-t border-gray-800 pt-2">
             <span className="text-gray-400">Est. Memory (Flyweight):</span>
             <span className="text-green-400">{memoryUsage.withFlyweight.toLocaleString()} units</span>
           </div>
           <p className="mt-2 text-gray-500 italic">Savings: {Math.round((1 - memoryUsage.withFlyweight/memoryUsage.withoutFlyweight)*100)}%</p>
        </div>
      </div>
      ...
      <div className="bg-[#e8f5e9] rounded-xl overflow-hidden relative min-h-[400px] border border-green-900/20 shadow-inner">
         ...
         {trees.length > 500 && (
            <div className="absolute inset-0 flex items-center justify-center bg-black/50 text-white font-bold backdrop-blur-sm">
               Rendering capped at 500 (DOM limitation),<br/>but {count} objects exist in memory!
            </div>
         )}
      </div>
    </div>
